...
void write_superblock(int fd) {
	...
	superblock.s_inodes_per_group  = NUM_INODES; //only have one block - will use all inodes
	superblock.s_mtime             = 0; /* Mount time - not sure about this one*/
...
void write_block_bitmap(int fd) {
	...
	u8 bytes[BLOCK_SIZE]={0}; //the size of the block - each char is one byte - use the different bits
	for(int i=0; i<BLOCK_SIZE; i++)
	{
		for(int j=0; j<8; j++)
		{
			...
			else if(((i*8)+j)>=(NUM_BLOCKS-1)) // only have 1024 blocks to use
			{
				bytes[i]+=power_2(j);
			}
		}
	}
	ssize_t size = sizeof(bytes);
...
void write_inode_bitmap(int fd) {
	...
	for(int i=0; i<BLOCK_SIZE; i++)
	{
		for(int j=0; j<8; j++)
		{
			...
			else if(((i*8)+j)>=(NUM_INODES)) // only have 128 inodes to use
			{
				bytes[i]+=power_2(j);
			}
		}
...
void write_inode_table(int fd) {
   ...
   root_inode.i_gid = 0;
   root_inode.i_links_count = 3; // "root/." "root/.." "lost-and-found/.."
...
struct ext2_inode hello_inode = {0};
  ...
  hello_inode.i_block[2] = 0x646C72;
...
void write_hello_world_file_block(int fd) {
	...
	char contents[12] = "Hello world\n";

	ssize_t size = sizeof(contents);
